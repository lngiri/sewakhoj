-- Migration 082: Fix UUID/Text type mismatch in push notification triggers
--
-- The bookings.service column is TEXT type, but services.id is UUID.
-- All comparisons between these must use explicit ::uuid casting.
-- Affected functions: notify_new_booking(), notify_booking_status_update()

BEGIN;

-- ============================================================================
-- 1. FIX: notify_new_booking()
--    Cast NEW.service::uuid when comparing to services.id
-- ============================================================================
...
BEGIN
  ...
  -- Get service name (cast to uuid because bookings.service is TEXT)
...
-- ============================================================================
-- 2. FIX: notify_booking_status_update()
--    Cast NEW.service::uuid when comparing to services.id
--    Also simplified: removed unnecessary auth.users lookup since
--    NEW.customer_id directly references auth.users.id
-- ============================================================================
