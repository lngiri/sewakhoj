      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          ...
          <div className="space-y-6">
            <Link href="/" className="flex items-center gap-3">
              ...
              <div translate="no">
                <div className="text-2xl font-black text-white tracking-tight">{siteConfig.name}</div>
            ...
            <div className="flex gap-4">
              <a href={siteConfig.socialFacebook} target="_blank" rel="noopener noreferrer" aria-label="Visit SewaKhoj on Facebook" className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center hover:bg-blue-600 transition-all group">
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm font-medium">
              ...
              <li><a href={`mailto:${siteConfig.email}`} className="flex items-center gap-3 hover:text-white transition-colors">✉️ {siteConfig.email}</a></li>
              <li><a href={`tel:${siteConfig.phone.replace(/\s/g, '')}`} className="flex items-center gap-3 hover:text-white transition-colors">📞 {siteConfig.phone}</a></li>
              <li className="flex items-center gap-3">📍 {siteConfig.address}</li>
            </ul>
          </div>
        </div>
        ...
        <div className="border-t border-white/5 pt-10 flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="text-sm text-white/60 font-bold">
            &copy; {new Date().getFullYear()} {siteConfig.name} Technologies Pvt. Ltd. All rights reserved.
          </p>
